...
@Component({
  selector: 'app-viajes-administracion',
  imports: [TableModule, CommonModule, DropdownComponent, FormsModule, CheckboxModule],
  templateUrl: './viajes-administracion.component.html',
  styleUrl: './viajes-administracion.component.scss'
})
export class ViajesAdministracionComponent {
  ...
  constructor(private dixiService: DexieService, 
    ...
    private dexiService: DexieService,
  ...
  async LimpiarViajes() {
    await this.dixiService.clearViajes();
    this.viajes.length=0;
    this.selectedViajes.length=0;
  }

  cargarFechaActual() {
    this.filtro.fecha = this.utilsService.formatDate3(new Date());
  }

  async getConductores() {
    const formato = [{ruc: this.usuario.ruc}]
    const conductores = await this.maestrasService.getConductores(formato)
    if(!!conductores.data && conductores.data.length>0) { 
      await this.dexiService.saveConductores(conductores.data)
      await this.ListarConductores()
    }
  }

  async ListarConductores() { 
    const conductores = await this.dexiService.showConductores();
  ...
  async recuperarViajes() {
    if(this.filtro.idconductor=='' || this.filtro.fecha=='') {
      this.alertService.showAlert('Alerta!','Debe seleccionar un chofer y una fecha','warning');
      return;
    }
    ...
    const formato = [
			{
				ruc: this.usuario.ruc,
				fecha: this.utilsService.formatoAnioMesDia(this.filtro.fecha),
				conductor: this.filtro.idconductor
			}
		]
    const viajes = await this.authService.getRecuperarViajes(formato);
    if (!!viajes) {
      await this.dexiService.saveViajes(viajes);
      this.ListarViajes();
    }
